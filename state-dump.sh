#!/usr/bin/env bash
# Primer Cliente — estado desplegado en un comando. Solo curl + grep.
set -uo pipefail
APP="https://primer-cliente.vercel.app"
OUT="primer-cliente-state.md"
CB="cb=$(date +%s)"
{
echo "# Primer Cliente — estado desplegado"
echo
...
RESP=$(curl -fsSL -m 90 -X POST "$APP/api/verdicto" -H "Content-Type: application/json" \
  -d '{"productId":"primercliente","texto":"sé editar videos, hago uñas, arreglo computadores, enseño matemáticas, cocino para eventos..."}' 2>/dev/null)
echo '```'
